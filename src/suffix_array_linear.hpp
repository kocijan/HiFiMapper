...
// Construction.pdf"
// https://github.com/kurpicz/saca-bench/blob/master/sa-is/sais.cpp
...
template <typename T> std::vector<uint32_t> suffix_array_linear(T &seq) {
  biosoup::Timer timer{};
  timer.Start();
  ...
  if (n == 1)
    return {0};
  timer.Stop();
  // std::cout << "PRETVARANJE: "<< timer.elapsed_time() << std::endl;
  timer.Start();
  std::vector<int> sa(n + 1);
  SA_IS((unsigned char *)s.c_str(), sa.data(), n + 1, 256, 1);
  timer.Stop();
  // std::cout << "KONSTRUKCIJA: "<< timer.elapsed_time() << std::endl;
  sa.erase(sa.begin());
